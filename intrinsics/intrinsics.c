...
int __CPU_DBRG_SEED__(void *buf, size_t len, int retry_max) {
    ...
    if (rem) {
        ...
        while (1) {
            block = (*_libdnml_ghw_ftable.hw_drbg)(&err);
            if (retry_cnt > retry_max) return -1;
            if (!err) { memcpy(p, &block, rem); break; } 
            ++retry_cnt;
            (*_libdnml_ghw_ftable.hw_shalt)();
        }
    } return 0;
...
int __CPU_TRNG_SEED__(void *buf, size_t len, int retry_max, bool crypt) {
    ...
    if (rem) {
        ...
        while (1) {
            ...
            if (retry_cnt > retry_max) return -1;
            if (!err) {
                if (!crypt) __MEMCPY_STD__(p, &block, rem);
...
static int ___ENTROPY_URANDOM(void *buf, size_t len, int retry_max) { // Unix Portable fallback
...
static int ___ENTROPY_LINUX(void *buf, size_t len, int retry_max) { // Linux
#if __OS_LINUX__
    ...
    while (remaining) {
        ...
        if (ret < 0) {
            if (errno == ENOSYS) return ___ENTROPY_URANDOM(buf, len);
            if (errno == EINTR) {
                if (retry_cnt > retry_max) return -1;
...
static int ___ENTROPY_DARWIN(void *buf, size_t len, int retry_max) { // MacOS and BSDs (Open, Net, Free)
    ...
    while (remain) {
        ...
        if (!ret) { ucbuf += n; remain -= n; continue; }
        // Failed
        ...
        if (errno == ENOSYS || errno == EPERM || errno == EACCES) {
            ...
            return ___ENTROPY_URANDOM(buf, len, retry_max);
        } 
        return -1;
    }
    return 0;
...
static int ___ENTROPY_IOS(void *buf, size_t len, int retry_max) { // iOS
    unsigned char *ucbuf = (unsigned char*)buf;
    size_t rem = len;
    while (rem) {
...
static int ___ENTROPY_WIN64(void *buf, size_t len) { // Windows (64 bit)
#if __OS_WIN64__
    NTSTATUS status;
...
static int __GET_ENTROPY_UNBL(void *buf, size_t len, int retry_max) {
#if __OS_LINUX__
    return ___ENTROPY_LINUX(buf, len, retry_max);
#elif __OS_IOS__
    return ___ENTROPY_IOS(buf, len, retry_max);
#elif __OS_MACOS__ || __OS_BSD__
    return ___ENTROPY_DARWIN(buf, len, retry_max);
#elif __OS_WIN64__
    return ___ENTROPY_WIN64(buf, len, retry_max);
#else
...
void __GET_ENTROPY_FAST(void* buf, size_t len) {
    ...
    // RDRAND FUNCTION HERE

    // 2. Non-blocking, OS Fall-back
    if (!__GET_ENTROPY_UNBL(buf, len, 3)) return;
...
void __GET_ENTROPY_PQC(void *buf, size_t len) {
    ...
    while (processed < len) {
        ...
        // Gathers via CPU instruction for Hardware pool
        __CPU_TRNG_SEED__(pool_hw, chunk, 128, true);
        if (__GET_ENTROPY_UNBL(pool_os, chunk, 16)) {
            uint64_t *jitter_ptr = (uint64_t *)pool_os;
            for (size_t i = 0; i < (chunk >> 3); ++i) jitter_ptr[i] = __jitter_harvest(2048);
            size_t tail = chunk & (3 - 1); // chunk % 8
            if (tail) { // Filling in the remaining spaces
                uint64_t last_jit = __jitter_harvest(2048);
                __MEMCPY_STRICT__(pool_os + (chunk - tail), &last_jit, tail);
            }
        }
