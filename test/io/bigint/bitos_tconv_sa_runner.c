...
scase ecases_nob[25] = { // Total: 3556 bytes - 3.556 KB
    ...
    { /* 1          | 0 (n = 0, sign = 1)                       | 0         | 1         | STR_INVALID_CAP                           */
    ...
    }, { /* 2       | 1 (n = 1, sign = -1)                      | 0         | 2         | STR_INVALID_CAP                           */
        .in = &(bitos_conv_in){
            .base = 0, .uppercase = false, .len = 0,
        ...
        .exp = {
            ...
            .cap = 20, .data.len = 20, .pstr = "-8446744073709551615"
        }
    },
    /* ------------------------------------------------------- EDGE CASES --------------------------------------------------------- */
    { /* 8          | -2^96 (n = 2, sign = -1)                  | 29        | 30        | STR_TRUNC_SUCCESS                         */
        .in = &(bitos_conv_in){
        ...
        .exp = {
            .type = STRING, .status = STR_TRUNC_SUCCESS,
            .cap = 29, .data.len = 29, .pstr = "-9228162514264337593543950336"
        }
    ...
    }, { /* 15      | SUPER SUPER SPARSE (n = 8) - fail edition | 130       | 135       | STR_TRUNC_SUCCESS                         */
         .in = &(bitos_conv_in){
            .base = 0, .uppercase = false, .len = 130,
            .x = { .limbs = case_10, .n = 8, .cap = 8, .sign = 1 }
        },
        .exp = {
            .type = STRING, .status = STR_TRUNC_SUCCESS, .cap = 130, .data.len = 130,
            .pstr = "8724295606890549323807888004534353641360687318060281490199180639"
                    "2881133979233261910507137635655607625216062661779335346016286146"
                    "56"
        }
        ...
        .exp = {
            .type = STRING, .status = STR_TRUNC_SUCCESS,
            .cap = 50, .data.len = 50, .pstr = "86360902046246736794993821988362198414833098878702"
        }
    }, { /* 17      | Dense -> Sparse (n = 5)                   | 60        | 78        | STR_TRUNC_SUCCESS                         */
         .in = &(bitos_conv_in){
            .base = 0, .uppercase = false, .len = 60,
            .x = { .limbs = case_17, .n = 5, .cap = 5, .sign = 1 }
        },
        .exp = {
            .type = STRING, .status = STR_TRUNC_SUCCESS, .cap = 60, .data.len = 60,
            .pstr = "423570985008687907853610267032561502502920958615344897851391"
        }
    }, { /* 18      | 2^192 (n = 4)                             | 25        | 58        | STR_TRUNC_SUCCESS                         */
         .in = &(bitos_conv_in){
        ...
        .exp = {
            .type = STRING, .status = STR_TRUNC_SUCCESS,
            .cap = 25, .data.len = 25, .pstr = "6416102355444464034512896"
        }
    }, { /* 19      | 2^256 - 1 (n = 4)                         | 77        | 78        | STR_TRUNC_SUCCESS                         */
         .in = &(bitos_conv_in){
        ...
        .exp = {
            ...
            .pstr = "15792089237316195423570985008687"
                    "90785326998466564056403945758400"
                    "7913129639935"
        }
    ...
    }, { /* 23      | -2^512 (n = 9, sign = -1)                 | 155       | 156       | STR_TRUNC_SUCCESS                         */
         .in = &(bitos_conv_in){
            .base = 0, .uppercase = false, .len = 155,
            .x = { .limbs = case_23, .n = 9, .cap = 9, .sign = -1 }
        },
        .exp = {
            .type = STRING, .status = STR_TRUNC_SUCCESS, .cap = 155, .data.len = 155,
                .pstr = "-340780792994259709957402499820584612747936582059239337772356144"
                        "3721764030073546976801874298166903427690031858186486050853753882"
                        "811946569946433649006084096"
        }
    }, { /* 24      | -(2^3072 - 1) (n = 48, sign = -1)         | 925       | 926       | STR_TRUNC_SUCCESS                          */
         .in = &(bitos_conv_in){
            .base = 0, .uppercase = false, .len = 925,
            .x = { .limbs = case_final, .n = 48, .cap = 48, .sign = -1 }
        },
        .exp = {
            .type = STRING, .status = STR_TRUNC_SUCCESS, .cap = 925, .data.len = 925,
            .pstr = "-809605995369958062859502533304574370686975176362895236661486152"
...
scase ecases_b[25] = { // 5968 bytes - 6kb ---> Rounded to 6016 bytes
    ...
    }, { /* 2       | 1 (n = 1, sign = -1)                      | 10        | 0         | 2         | STR_INVALID_CAP                           */
        .in = &(bitos_conv_in){
            .base = 10, .uppercase = false, .len = 0,
            .x = { .limbs = &one, .n = 1, .cap = 1, .sign = -1 }
        }, INVAL_STR(STR_INVALID_CAP, 0),
