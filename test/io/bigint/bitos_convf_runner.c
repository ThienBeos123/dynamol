...
scase ecases[25] = { // 2270 bytes of memory ---> Rounded up to 2304
    ...
    { /* 1          | 0 (n = 0)                         | 2             | "0b0"                                                 */
        ...
        .in = &(bitos_conv_in){
            .base = 16, .uppercase = true, .len = STR_OUT_CAP,
    ...
    { /* 8          | 2^64 (n = 2)                      | 2             | "0b100000000000...000000000000" (truncated)           */
        ...
        .in = &(bitos_conv_in){
            .base = 8, .uppercase = true, .len = STR_OUT_CAP,
        ...
        .in = &(bitos_conv_in){
            .base = 3, .uppercase = true, .len = STR_OUT_CAP,
        ...
        .in = &(bitos_conv_in){
            .base = 10, .uppercase = true, .len = STR_OUT_CAP,
            .x = { .limbs = &small_mulval[3], .cap = 1, .n = 1, .sign = -1 }
        },
        ...
        .in = &(bitos_conv_in){
            .base = 2, .uppercase = true, .len = STR_OUT_CAP,
        ...
        .in = &(bitos_conv_in){
            .base = 32, .uppercase = true, .len = STR_OUT_CAP,
        ...
        .in = &(bitos_conv_in){
            .base = 18, .uppercase = true, .len = STR_OUT_CAP,
        ...
        .in = &(bitos_conv_in){
            .base = 64, .uppercase = true, .len = STR_OUT_CAP,
            .x = { .limbs = case_final, .cap = 48, .n = 48, .sign = -1 }
        },
        .exp = {
...
int main(int argc, char **argv) { _libdnml_init();
    ...
    create_str_suite(&to_strf_suite, "to_strf - BigInt Conversion", 
        conv_ecount, rcount, ecases, INVERSE, ebuf_slices[0], 
    ...
    create_str_suite(&tto_strf_suite, "tto_strf - BigInt Conversion",
        conv_ecount, rcount, ecases, INVERSE, ebuf_slices[1],
        "logs/bigint_to_strf.txt", &conv_ectx, &conv_rcon,
        &convf_rconfig, &conv_rstate
    ); tto_strf_suite.cap_mode = ENOUGH;
    fill_suite_rinv(&tto_strf_suite,
        &_bitos_convf_ingen, &exec_bitos_tto_strf,
        &inv_bitos_conv_b, &stat_bitos_conv_b, 
        &cmp_inv_bitos_conv, &fmt_in_to_strf, &fmt_recon_bitos,
        &_bitos_conv_inlink, &_bitos_conv_insize,
        &_bitos_recon_linker, &_bitos_recon_size,
    ...
    conv_suite_arr[0] = to_strf_suite; conv_suite_arr[1] = tto_strf_suite;
