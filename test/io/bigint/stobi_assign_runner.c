...
scase ecases_bprefix[27] = {
    ...
    { /* 1.         |   ""                              ---->   STR_EMPTY                           */
        .in = &(stobi_assign_in){ .str = "", .len = 0, .base = 0 },
        .exp = { .type = BIGINT, .status = STR_EMPTY, .cap = 0, INVAL_BI() }
    }, { /* 2.      |   "  "                            ---->   STR_EMPTY                           */
        .in = &(stobi_assign_in){ .str = "", .len = 0, .base = 0 },
        .exp = { .type = BIGINT, .status = STR_EMPTY, .cap = 0, INVAL_BI() }
    }, { /* 3.      |   "+"                             ---->   STR_INCOMPLETE                      */
        .in = &(stobi_assign_in){ .str = "  ", .len = 3, .base = 0 },
        .exp = { .type = BIGINT, .status = STR_INCOMPLETE, .cap = 0, INVAL_BI() }
    }, { /* 4.      |   "-"                             ---->   STR_INCOMPLETE                      */
        .in = &(stobi_assign_in){ .str = "+", .len = 2, .base = 0 },
        .exp = { .type = BIGINT, .status = STR_INCOMPLETE, .cap = 0, INVAL_BI() }
    }, { /* 5.      |   "-0"                            ---->   STR_INVALID_SIGN                    */
        .in = &(stobi_assign_in){ .str = "-0", .len = 3, .base = 0 },
    ...
    { /* 6.         |   "+0"                            ---->   STR_SUCCESS (0)                     */
        ...
        .in = &(stobi_assign_in){ .str = "0", .len = 2, .base = 0 },
        .exp = {
            .type = BIGINT, .status = STR_SUCCESS, .cap = 0,
            .data.bi = { .limbs = &multi_val[4], .n = 0, .cap = 1, .sign = 1 }
        }
    }, { /* 8.      |   "000000"                        ---->   STR_SUCCESS (0)                     */
        .in = &(stobi_assign_in){ .str = "000000", .len = 7, .base = 0 },
        .exp = {
            .type = BIGINT, .status = STR_SUCCESS, .cap = 0,
            .data.bi = { .limbs = &multi_val[4], .n = 0, .cap = 1, .sign = 1 }
        }
        ...
        .in = &(stobi_assign_in){ .str = "18446744073709551616", .len = 21, .base = 0 },
        .exp = { 
            .type = BIGINT, .status = STR_SUCCESS, .cap = 0, 
    ...
    { /* 12.        |   "1234abcd"                      ---->   STR_INVALID_DIGIT                   */
        .in = &(stobi_assign_in){ .str = "1234abcd", .len = 9, .base = 0 },
        .exp = { .type = BIGINT, .status = STR_INVALID_DIGIT, .cap = 0, INVAL_BI() }
    }, { /* 13.     |   "1234    "                      ---->   STR_INVALID_DIGIT                   */
        .in = &(stobi_assign_in){ .str = "1234    ", .len = 9, .base = 0 },
        .exp = { .type = BIGINT, .status = STR_INVALID_DIGIT, .cap = 0, INVAL_BI() }
    }, { /* 14.     |   "abcd1234"                      ---->   STR_INVALID_DIGIT                   */
        .in = &(stobi_assign_in){ .str = "abcd1234", .len = 9, .base = 0 },
        .exp = { .type = BIGINT, .status = STR_INVALID_DIGIT, .cap = 0, INVAL_BI() }
    }, { /* 15.     |   "1234_5678"                     ---->   STR_INVALID_DIGIT                   */
        .in = &(stobi_assign_in){ .str = "1234_5678", .len = 10, .base = 0 },
        .exp = { .type = BIGINT, .status = STR_INVALID_DIGIT, .cap = 0, INVAL_BI() }
    }, 
    /* ----------------------------------- BASE-SPECIFIC FAILURE ---------------------------------- */
    { /* 16.        |   "0x"                            ---->   STR_INCOMPLETE                      */
        .in = &(stobi_assign_in){ .str = "0x", .len = 3, .base = 0 },
        .exp = { .type = BIGINT, .status = STR_INCOMPLETE, .cap = 0, INVAL_BI() }
    }, { /* 17.     |   "0xFG"                          ---->   STR_INVALID_DIGIT                   */
        .in = &(stobi_assign_in){ .str = "0xFG", .len = 0, .base = 0 },
        .exp = { .type = BIGINT, .status = STR_INVALID_DIGIT, .cap = 0, INVAL_BI() }
    }, { /* 18.     |   "0b102"                         ---->   STR_INVALID_DIGIT                   */
        .in = &(stobi_assign_in){ .str = "0b102", .len = 6, .base = 0 },
        .exp = { .type = BIGINT, .status = STR_INVALID_DIGIT, .cap = 0, INVAL_BI() }
    }, { /* 19.     |   "0o89"                          ---->   STR_INVALID_DIGIT                   */
        .in = &(stobi_assign_in){ .str = "0o89", .len = 5, .base = 0 },
        .exp = { .type = BIGINT, .status = STR_INVALID_DIGIT, .cap = 0, INVAL_BI() }
    }, { /* 20.     |   "00x123"                        ---->   STR_INVALID_BASE_PREFIX             */
        .in = &(stobi_assign_in){ .str = "00x123", .len = 0, .base = 0 },
        .exp = { .type = BIGINT, .status = STR_INVALID_BASE_PREFIX, .cap = 0, INVAL_BI() }
    }, { /* 21.     |   "0{11)1234A"                    ---->   STR_INVALID_BASE_PREFIX             */
        .in = &(stobi_assign_in){ .str = "0{11)1234A", .len = 11, .base = 0 },
        .exp = { .type = BIGINT, .status = STR_INVALID_BASE_PREFIX, .cap = 0, INVAL_BI() }
    },
    /* ---------------------------------- BASE-SPECIFIC SUCCESSES --------------------------------- */
    { /* 22.        |   "0x0"                           ---->   STR_SUCCESS (0)                     */
        .in = &(stobi_assign_in){ .str = "0x0", .len = 4, .base = 0 },
        .exp = { 
            .type = BIGINT, .status = STR_SUCCESS, .cap = 0,
            .data.bi = { .limbs = &multi_val[4], .n = 0, .cap = 1, .sign = 1 }
        }
    }, { /* 23.     |   "0xFF"                          ---->   STR_SUCCESS (255)                   */
        .in = &(stobi_assign_in){ .str = "0xFF", .len = 4, .base = 0 },
        .exp = { 
            .type = BIGINT, .status = STR_SUCCESS, .cap = 0,
            .data.bi = { .limbs = &multi_val[1], .n = 1, .cap = 1, .sign = 1 }
        }
    }, { /* 24.     |   "0b101010"                      ---->   STR_SUCCESS (42)                    */
        .in = &(stobi_assign_in){ .str = "0b101010", .len = 9, .base = 0 },
        .exp = { 
            .type = BIGINT, .status = STR_SUCCESS, .cap = 0,
            .data.bi = { .limbs = &multi_val[2], .n = 1, .cap = 1, .sign = 1 }
        }
    }, { /* 25.     |   "0o77"                          ---->   STR_SUCCESS (63)                    */
        .in = &(stobi_assign_in){ .str = "0o77", .len = 5, .base = 0 },
        .exp = { 
            .type = BIGINT, .status = STR_SUCCESS, .cap = 0,
        ...
        .in = &(stobi_assign_in){ .str = "0{7}123456", .len = 11, .base = 0 },
        .exp = { 
            .type = BIGINT, .status = STR_SUCCESS, .cap = 0,
...
scase ecases_base[27] = {
    ...
    /* ------------------------------ PREPARSE LEXICAL FAILURE CASE ------------------------------- */ 
    { /* 1.         |   ""                              ---->   STR_EMPTY                           */
        .in = &(stobi_assign_in){ .str = "", .len = 0, .base = 10 },
        .exp = { .type = BIGINT, .status = STR_EMPTY, .cap = 0, INVAL_BI() }
    }, { /* 2.      |   "  "                            ---->   STR_EMPTY                           */
        .in = &(stobi_assign_in){ .str = "", .len = 0, .base = 10 },
        .exp = { .type = BIGINT, .status = STR_EMPTY, .cap = 0, INVAL_BI() }
    }, { /* 3.      |   "+"                             ---->   STR_INCOMPLETE                      */
        .in = &(stobi_assign_in){ .str = "  ", .len = 3, .base = 10 },
        .exp = { .type = BIGINT, .status = STR_INCOMPLETE, .cap = 0, INVAL_BI() }
    }, { /* 4.      |   "-"                             ---->   STR_INCOMPLETE                      */
        .in = &(stobi_assign_in){ .str = "+", .len = 2, .base = 10 },
        .exp = { .type = BIGINT, .status = STR_INCOMPLETE, .cap = 0, INVAL_BI() }
    }, { /* 5.      |   "-0"                            ---->   STR_INVALID_SIGN                    */
        .in = &(stobi_assign_in){ .str = "-0", .len = 3, .base = 10 },
        .exp = { .type = BIGINT, .status = STR_INVALID_SIGN, .cap = 0, INVAL_BI() }
    },
    /* --------------------------------- ON-THE-EDGE SUCCESS CASE --------------------------------- */ 
    { /* 6.         |   "+0"                            ---->   STR_SUCCESS (0)                     */
        ...
        .in = &(stobi_assign_in){ .str = "0", .len = 2, .base = 10 },
        .exp = {
            .type = BIGINT, .status = STR_SUCCESS, .cap = 0,
            .data.bi = { .limbs = &multi_val[4], .n = 0, .cap = 1, .sign = 1 }
        }
    }, { /* 8.      |   "000000"                        ---->   STR_SUCCESS (0)                     */
        .in = &(stobi_assign_in){ .str = "000000", .len = 7, .base = 10 },
        .exp = {
            .type = BIGINT, .status = STR_SUCCESS, .cap = 0,
            .data.bi = { .limbs = &multi_val[4], .n = 0, .cap = 1, .sign = 1 }
        }
        ...
        .in = &(stobi_assign_in){ .str = "18446744073709551616", .len = 21, .base = 10 },
        .exp = { 
            .type = BIGINT, .status = STR_SUCCESS, .cap = 0, 
            .data.bi = { .limbs = big_limbs, .n = 2, .cap = 2, .sign = 1 }
        }
    ...
    /* -------------------------- NUMERICAL-SEGMENT DIGIT-BASED FAILURE --------------------------- */
    { /* 12.        |   "1234abcd"                      ---->   STR_INVALID_DIGIT                   */
        .in = &(stobi_assign_in){ .str = "1234abcd", .len = 9, .base = 10 },
        .exp = { .type = BIGINT, .status = STR_INVALID_DIGIT, .cap = 0, INVAL_BI() }
    }, { /* 13.     |   "1234    "                      ---->   STR_INVALID_DIGIT                   */
        .in = &(stobi_assign_in){ .str = "1234    ", .len = 9, .base = 10 },
        .exp = { .type = BIGINT, .status = STR_INVALID_DIGIT, .cap = 0, INVAL_BI() }
    }, { /* 14.     |   "abcd1234"                      ---->   STR_INVALID_DIGIT                   */
        .in = &(stobi_assign_in){ .str = "abcd1234", .len = 9, .base = 10 },
        .exp = { .type = BIGINT, .status = STR_INVALID_DIGIT, .cap = 0, INVAL_BI() }
    }, { /* 15.     |   "1234_5678"                     ---->   STR_INVALID_DIGIT                   */
        .in = &(stobi_assign_in){ .str = "1234_5678", .len = 10, .base = 10 },
        .exp = { .type = BIGINT, .status = STR_INVALID_DIGIT, .cap = 0, INVAL_BI() }
    }, 
    /* -------------------------------------------------------------------------------------------- */
...
int main(int argc, char **argv) {
    ...
    create_str_suite(&get_str_suite, "bigInt_get_str - String Assignment", 
        ...
        "../logs/bigInt_get_str.txt", assign_ectx, &assign_rctx
    );
