...
static const bigInt global_cases[CASE_CNT] = {
...
int main(void) { _libdnml_init();
    ...
    limb_t *dst_buffer = (limb_t *)malloc(4 * sizeof(limb_t)); assert(dst_buffer != NULL);
    fputs("===================================================================\n", stdout);
    fputs("     RUNNING INTEGRATED UNIT TESTS - BIGNUM COPY UTILITIES         \n", stdout);
    fputs("===================================================================\n", stdout);
    fputs("---- __BIGINT_INTERNAL_COPY__ -----\n", stdout);
    for (int i = 0; i < CASE_CNT; i++) { total_tests++;
        const bigInt *src = &global_cases[i];
        bigInt dst = { .n = 0, .cap = MAX_SIZE_T, .limbs = dst_buffer, .sign = 1 };
        memset(dst_buffer, 0, MAX_SIZE_T * sizeof(limb_t));
