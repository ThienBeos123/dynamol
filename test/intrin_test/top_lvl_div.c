...
int main(void) { _libdnml_init();
    ...
    for (int i = 0; i < 32; i++) {
        ...
        quot = __DIV_HELPER_UI64__(lo, hi, div, &rem, &ovf_check); total_tests++;
        if (quot == exp_quot && rem == exp_rem && ovf_check == exp_ovf) passed_tests++;
        else printf(
            "[FAIL] __DIV_HELPER_UI64__   "ylw"|"esc" Case %3d "ylw"|"esc" Lo: 0x%016" PRIX64 " Hi: 0x%016" PRIX64 " Div: 0x%016" PRIX64
    ...
    for (int i = 0; i < 32; i++) {
        ...
        // Portable _cintrin baseline
        quot = __CRT_DIV_U128__(lo, hi, div, &rem, &ovf_check); total_tests++;
        if (quot == exp_quot && rem == exp_rem && ovf_check == exp_ovf) passed_tests++;
        else printf(
            "[FAIL] __CRT_DIV_U128__      "ylw"|"esc" Case %3d "ylw"|"esc" Lo: 0x%016" PRIX64 " Hi: 0x%016" PRIX64 " Div: 0x%016" PRIX64
            " "ylw"|||"esc" Equot: 0x%016" PRIX64 " Erem: 0x%016" PRIX64 " Eovf: %1" PRIu8
            " "ylw"|||"esc" Oquot: 0x%016" PRIX64 " Orem: 0x%016" PRIX64 " Oovf: %1" PRIu8 "\n", 
            i + 1, lo, hi, div, exp_quot, exp_rem, exp_ovf, quot, rem, ovf_check
        );
