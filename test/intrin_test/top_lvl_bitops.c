...
int main(void) { _libdnml_init();
    ...
    for (int i = 0; i < 60; i++) {
        ...
        uint64_t res;
        // Perf-based Verification
        res = __BSWAP_UI64__(input);
        total_tests++; if (res == expected) passed_tests++;
        else printf(
            "[FAIL] __BSWAP_UI64__     | Case %2d | Input: 0x%016" PRIX64 
            " | Exp: 0x%016" PRIX64 " | Got: 0x%016" PRIX64 "\n",
            i + 1, input, expected, res
        );
        // Crypto-based Verification
        res = __CRT_BSWAP_UI64__(input);
        total_tests++; if (res == expected) passed_tests++;
        else printf(
            "[FAIL] __CRT_BSWAP_UI64__ | Case %2d | Input: 0x%016" PRIX64 
            " | Exp: 0x%016" PRIX64 " | Got: 0x%016" PRIX64 "\n",
            i + 1, input, expected, res
        );
    }
    ...
    for (int i = 0; i < 60; i++) {
        ...
        uint8_t res; 
        // Perf-based Verification
        res = __CLZ_UI64__(input);
        total_tests++; if (res == expected) passed_tests++;
        else printf("[FAIL] __CLZ_UI64__       "
            "| Case %2d | Input: 0x%016" PRIX64 " | Exp: %u | Got: %u\n", 
            i + 1, input, expected, res
        );
        // Crypto-based Verification
        res = __CRT_CLZ_UI64__(input);
        total_tests++; if (res == expected) passed_tests++;
        else printf("[FAIL] __CRT_CLZ_UI64__   "
            "| Case %2d | Input: 0x%016" PRIX64 " | Exp: %u | Got: %u\n", 
            i + 1, input, expected, res
        );
    }
    ...
    for (int i = 0; i < 60; i++) {
        ...
        uint8_t res;
        // Perf-based Verification
        res = __CTZ_UI64__(input);
        total_tests++; if (res == expected) passed_tests++;
        else printf("[FAIL] __CTZ_UI64__       "
            "| Case %2d | Input: 0x%016" PRIX64 " | Exp: %u | Got: %u\n", 
            i + 1, input, expected, res
        );
        // Crypto-based Verification
        res = __CRT_CTZ_UI64__(input);
        total_tests++; if (res == expected) passed_tests++;
        else printf("[FAIL] __CRT_CTZ_UI64__   "
            "| Case %2d | Input: 0x%016" PRIX64 " | Exp: %u | Got: %u\n", 
            i + 1, input, expected, res
        );
    }
    ...
    for (int i = 0; i < 60; i++) {
        ...
        uint8_t res;
        // Perf-based verification
        res = __PCNT_UI64__(input);
        total_tests++; if (res == expected) passed_tests++;
        else printf("[FAIL] __PCNT_UI64__       "
            "| Case %2d | Input: 0x%016" PRIX64 " | Exp: %u | Got: %u\n", 
            i + 1, input, expected, res
        );
        // Crypto-based verification
        res = __CRT_PCNT_UI64__(input);
        total_tests++; if (res == expected) passed_tests++;
        else printf("[FAIL] __CRT_PCNT_UI64__   "
            "| Case %2d | Input: 0x%016" PRIX64 " | Exp: %u | Got: %u\n", 
            i + 1, input, expected, res
        );
    }
