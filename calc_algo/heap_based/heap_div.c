...
static void __heap_burk_3b2(
    ...
    bigInt *alloc_list[3]; uint8_t alloc_cnt = 0;
    BIHEAP_TEMP(c, B->n, echeck, err, alloc_list, alloc_cnt,);
    BIHEAP_TEMP(iq, a1->n + a2->n, echeck, err, alloc_list, alloc_cnt,);
    __BIHEAP_BURNIKEL__(a1, a2, b1, q, &c, &rec_err); HEAP_OOM(rec_err, err, alloc_list, alloc_cnt,);

    BIHEAP_TEMP(d, (iq.n + b2->n), echeck, err, alloc_list, alloc_cnt,);
    uint64_t a[1] = {1}; bigInt one = {.limbs = a, .sign = 1, .n = 1, .cap = 1};
    __BIHEAP_MUL_DISP__(&iq, b2, &d, &echeck); HEAP_OOM(rec_err, err, alloc_list, alloc_cnt,);
...
void __BIHEAP_KNUTH_D__(const bigInt *a, const bigInt *b, bigInt *quot, bigInt *rem, dnml_status *err) {
    ...
    size_t m = a->n, n = b->n; bigInt *alloc_list[2]; uint8_t alloc_cnt = 0;
    BIHEAP_TEMP(a_copy, m + 1, echeck, err, alloc_list, alloc_cnt,);
    BIHEAP_TEMP(b_copy, n, echeck, err, alloc_list, alloc_cnt,);
    ...
    for (size_t j = m - n + 1; j > 0; --j) {
        ...
        if (overflow_check) { _free_alloc_list(alloc_list, alloc_cnt); *err = DARENA_OVERFLOW; return; }
...
void __BIHEAP_BURNIKEL__(
    ...
    if (AH->n + AL->n <= (BIGINT_SHORT << 1) && b->n <= BIGINT_SHORT) {
        dnml_status echeck = BIGINT_SUCCESS; bigInt *alloc_list[1]; uint8_t alloc_cnt = 0;
        BIHEAP_TEMP(a, AH->n + AL->n, echeck, err, alloc_list, alloc_cnt,);
    ...
    dnml_status echeck = BIGINT_SUCCESS, rec_err = BIGINT_SUCCESS;
    bigInt *alloc_list[3]; uint8_t alloc_cnt = 0;
    BIHEAP_TEMP(q1, (k << 1), echeck, err, alloc_list, alloc_cnt,);
    BIHEAP_TEMP(q2, (k << 1), echeck, err, alloc_list, alloc_cnt,);
    BIHEAP_TEMP(r,  (k << 1), echeck, err, alloc_list, alloc_cnt,);
    __heap_burk_3b2(
        &a1, &a2, &a3,  // Dividends
        &b1, &b2, b,    // Divisors
        &q1, &r,  /* Quotient + Remainders */ &rec_err
    ); HEAP_OOM(rec_err, err, alloc_list, alloc_cnt,);
