...
void __BIHEAP_SCHOOLBOOK__(PCONST_BIGINT a, PCONST_BIGINT b, P_BIGINT res) {
    bigInt tmp_res = {0}; __BIGINT_INTERNAL_LINIT__(&tmp_res, a->n + b->n);
    ...
    for (size_t i = 0; i < a->n; ++i) {
        ...
        for (size_t j = 0; j < b->n; ++j) {
            ...
            uint64_t sum = tmp_res.limbs[i + j] + low; uint8_t c1 = (sum < low);
            uint64_t sum2 = sum + carry; uint8_t c2 = (sum2 < carry);
            carry = high + (c1 + c2); tmp_res.limbs[i + j] = sum2;
        }
        // Properly ripple the final carry down the line to prevent the domino effect
        size_t k = i + b->n;
        while (carry) {
            uint64_t sum = tmp_res.limbs[k] + carry;
            carry = (sum < carry); // Capture any new carry generated by this addition
            tmp_res.limbs[k] = sum; /**/ k++;
        }
    } tmp_res.n = a->n + b->n; 
    __BIGINT_INTERNAL_TRIM_LZ__(&tmp_res); 
    __BIGINT_INTERNAL_MOVE__(res, &tmp_res);
}
...
void __BIHEAP_KARATSUBA__(PCONST_BIGINT x, PCONST_BIGINT y, P_BIGINT res, dnml_status *err) {
    ...
    bigInt x1 = {.limbs = x->limbs + m, .n = x1_range, .cap = x1_range};
    bigInt y0 = {.limbs = y->limbs,     .n = y0_range, .cap = y0_range};
    bigInt y1 = {.limbs = y->limbs + m, .n = y1_range, .cap = y1_range};
...
void __BIHEAP_TOOM_3__(PCONST_BIGINT m, PCONST_BIGINT n, P_BIGINT res, dnml_status *err) {
    ...
    */ q_neg2.cap = q_neg2.n;
    ...
    echeck = __BIGINT_INTERNAL_ENSCAP__(&r1, (k << 1) + 8); HEAP_OOM(echeck, err, early_free, early_cnt,);
    echeck = __BIGINT_INTERNAL_ENSCAP__(&r_neg1, (k << 1) + 7); HEAP_OOM(echeck, err, early_free, early_cnt,);
    echeck = __BIGINT_INTERNAL_ENSCAP__(&r_neg2, (k << 1) + 7); HEAP_OOM(echeck, err, early_free, early_cnt,);
    echeck = __BIGINT_INTERNAL_ENSCAP__(&rinf, (m2size + n2size) + 1); HEAP_OOM(echeck, err, early_free, early_cnt,);
    ...
    q_neg2.cap = m->n + n->n; q_neg2.n = 0; q_neg2.sign = 1; 
    __BIGINT_ADD_SHIFT__(&q_neg2, &rinf, (k << 2)); // final_res += (rinf <<< 4k)
    __BIGINT_ADD_SHIFT__(&q_neg2, &r_neg2, k * 3); // final_res += (r_neg2 <<< 3k)
    __BIGINT_ADD_SHIFT__(&q_neg2, &r_neg1, (k << 1)); // final_res += (r_neg1 <<< 2k)
    __BIGINT_ADD_SHIFT__(&q_neg2, &r1, k); // final_res += (r1 <<< 1k)
