...
void __BIHEAP_ASYM_KARAT__(PCONST_BIGINT x, PCONST_BIGINT y, P_BIGINT res, dnml_status *err) {
    ...
    for (size_t i = 0; i < splits; ++i) {
        ...
        if ((Bsize <= BIGINT_SCHOOLBOOK || curr_slice <= BIGINT_SCHOOLBOOK)) { 
            ...
            __BIHEAP_ASYM_MUL_DISP__(beta, &window, &z2, &echeck); HEAP_OOM(echeck, err, early_free, early_cnt,);
            __BIGINT_ADD_SHIFT__(&tmp_res, &z2, offset); continue; // (tmp_res <<<= slice) + tmp
        }
        if (i == splits - 1) {
...
void __BIHEAP_TOOM_3__(PCONST_BIGINT m, PCONST_BIGINT n, P_BIGINT res, dnml_status *err) {
    ...
    */
    ...
    __BIGINT_INTERNAL_ENSCAP__(&rinf, m2size + n2size); HEAP_OOM(echeck, err, early_free, early_cnt,);
    ...
    */
    ...
    for (size_t i = 0;i < splits; ++i) {
        ...
        if (i == splits - 1) {
            ...
            n1size = (curr_slice > k) ? curr_slice - k : 0; // Maximum = k
            ...
            r_neg2.cap = (k << 1) + 10; /**/ rinf.cap = m2size + n2size + 4;
        }
        /* ---------------- 1. SETUP & SPLITTING ---------------- */
        bigInt m0 = {.limbs = window.limbs,            .n = k,      .cap = k};
        bigInt m1 = {.limbs = window.limbs + k,        .n = m1size, .cap = m1size};
        bigInt m2 = {.limbs = window.limbs + (k << 1), .n = m2size, .cap = m2size};
        bigInt n1 = {.limbs = beta->limbs + k,         .n = k,      .cap = k};
        bigInt n0 = {.limbs = beta->limbs,             .n = n1size, .cap = n1size};
        ...
        */
        ...
        __BIGINT_ADD_SHIFT__(&q_neg2, &r1, 1); // final_res += (r1 <<< 1)
        __BIGINT_ADD_WC__(&q_neg2, &q_neg2, &r0); // final_res += r0
        __BIGINT_ADD_SHIFT__(&tmp_res, &q_neg2, offset); // Accumulate into the final result (like truly final)
    } __BIGINT_INTERNAL_MOVE__(res, &tmp_res); _free_alloc_list(alloc_list, alloc_cnt); *err = BIGINT_SUCCESS;
}
