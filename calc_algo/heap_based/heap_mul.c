...
void __BIHEAP_ASYM_KARAT__(PCONST_BIGINT x, PCONST_BIGINT y, P_BIGINT res, dnml_status *err) {
    ...
    bigInt *alloc_list[5] = {0}, *early_free[6]; 
    ...
    BIHEAP_TEMP(tmp1, max(x0_range, x1_range) + 1, echeck, err, early_free, early_cnt, alloc_list, alloc_cnt,);
    BIHEAP_TEMP(tmp2, max(y0_range, y1_range) + 1, echeck, err, early_free, early_cnt, alloc_list, alloc_cnt,);
    BIHEAP_TEMP(z0, x0_range + y0_range, echeck, err, early_free, early_cnt, alloc_list, alloc_cnt,);
    BIHEAP_TEMP(z1, z1_size, echeck, err, early_free, early_cnt, alloc_list, alloc_cnt,);
    BIHEAP_TEMP(z2, z2_size, echeck, err, early_free, early_cnt, alloc_list, alloc_cnt,);
    ...
    for (size_t i = 0; i < splits; ++i) {
        ...
        if (
            ...
            __BIHEAP_SCHOOLBOOK__(beta, &window, &z2);
...
void __BIHEAP_ASYM_TOOM_3__(PCONST_BIGINT m, PCONST_BIGINT n, P_BIGINT res, dnml_status *err) {
    ...
    */
    ...
    BIHEAP_TEMP(q_neg2,  k + 2, echeck, err, early_free, early_cnt, alloc_list, alloc_cnt,);
    /* ------------ POINT-WISE MULTIPLICATION ------------
    *   +) r(0)   = p(0)   * q(0)       ---> Cap: 2k
    *   +) r(1)   = p(1)   * q(1)       ---> Cap: 2k + 4 (original) --> 2k + 9 (interpolation - r1 + llshift)
    ...
    *   +) r(inf) = p(inf) * q(inf)     ---> Cap: 2k (original) ---> 2k + 4 (bit-shifts accounted)
    */
    BIHEAP_TEMP(r0,     (k << 1),       echeck, err, early_free, early_cnt, alloc_list, alloc_cnt,);
    BIHEAP_TEMP(r1,     (k << 1) + 9,   echeck, err, early_free, early_cnt, alloc_list, alloc_cnt,);
    BIHEAP_TEMP(r_neg1, (k << 1) + 9,   echeck, err, early_free, early_cnt, alloc_list, alloc_cnt,);
    BIHEAP_TEMP(r_neg2, (k << 1) + 10,  echeck, err, early_free, early_cnt, alloc_list, alloc_cnt,);
    BIHEAP_TEMP(rinf,    m2size + n2size + 4, echeck, err, early_free, early_cnt, alloc_list, alloc_cnt,);
    BIHEAP_TEMP(final_res, (k << 1) + 14, echeck, err, early_free, early_cnt, alloc_list, alloc_cnt,);
...
void __BIHEAP_MUL_DISP__(PCONST_BIGINT a, PCONST_BIGINT b, P_BIGINT res, dnml_status *err) {
    ...
    else if (min(a->n, b->n) * 2 <= max(a->n, b->n)) __BIHEAP_ASYM_MUL_DISP__(a, b, res, err);
