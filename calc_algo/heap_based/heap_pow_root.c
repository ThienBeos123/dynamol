...
void __BIHEAP_BINARY_EXP__(bigInt *const res, const bigInt *const base, uint64_t exp, dnml_status *err) {
    ...
    BIHEAP_TEMP(tmp_res, base->n * exp, echeck, err, alloc_list, alloc_cnt,); tmp_res.limbs[0] = 1; tmp_res.n = 1;
    BIHEAP_TEMP(tmp_base, base->n * exp, echeck, err, alloc_list, alloc_cnt,);
    memcpy(tmp_base.limbs, base->limbs, base->n * U64_BYTES); tmp_base.n = base->n;
    // MAIN OPERATION
    while (exp) {
        if (exp & 1) {
            __BIHEAP_MUL_DISP__(&tmp_res, &tmp_base, &tmp_res, &echeck); HEAP_OOM(echeck, err, alloc_list, alloc_cnt,);
        } __BIHEAP_MUL_DISP__(&tmp_base, &tmp_base, &tmp_base, &echeck); HEAP_OOM(echeck, err, alloc_list, alloc_cnt,);
        exp >>= 1;
...
void __BIHEAP_FIXED__(bigInt *const res, const bigInt *const base, uint64_t exp, uint8_t k, dnml_status *err) {
    /* --- 1. SETUP ---- */ dnml_status echeck; bigInt *alloc_list[2]; uint8_t alloc_cnt = 0;
    size_t table_size = 1 << (k - 1); bigInt table[table_size]; table[0] = *base;
    BIHEAP_TEMP(x2, (base->n << 1), echeck, err, alloc_list, alloc_cnt,);
    __BIHEAP_MUL_DISP__(base, base, &x2, &echeck);  HEAP_OOM(echeck, err, alloc_list, alloc_cnt,);

    //* ---- 2. TABLE PRECOMPUTATION ----- *//
    for (size_t i = 1; i < table_size; ++i) {
        __BIGINT_INTERNAL_LINIT__(&table[i], base->n * ((i << 1) + 1)); HEAP_OOM(echeck, err, alloc_list, alloc_cnt,);
        __BIHEAP_MUL_DISP__(&table[i-1], &x2, &table[i], &echeck); HEAP_OOM(echeck, err, alloc_list, alloc_cnt,);
    }

    //* ----- 3. MAIN LOOP ------ *//
    uint8_t chunk_count = (uint8_t)((U64_BITS - __CLZ_UI64__(exp)) >> 2) + 1;
    BIHEAP_TEMP(tmp_res, (base->n * exp), echeck, err, alloc_list, alloc_cnt,);
    ...
    for (uint8_t i = chunk_count - 1; i != (uint8_t)-1; --i) {
        ...
        for (uint8_t j = 1; j <= chunk_count - s; ++j) {
            __BIHEAP_MUL_DISP__(&tmp_res, &tmp_res, &tmp_res, &echeck); HEAP_OOM(echeck, err, alloc_list, alloc_cnt,);
...
void __BIHEAP_SLIDING__(bigInt *const res, const bigInt *const base, uint64_t exp, uint8_t k, dnml_status *err) {
    ...
    while (curr_pos != (uint8_t)-1) {
        ...
        if (!curr_bit) { 
            ...
            HEAP_OOM(echeck, err, alloc_list, alloc_cnt,); curr_pos = s - 1;
        }
    } __BIGINT_INTERNAL_COPY__(res, &tmp_res); _free_alloc_list(alloc_list, alloc_cnt); *err = BIGINT_SUCCESS;
}
void __BIHEAP_HERON__(bigInt *const res, const bigInt *const a, dnml_status *err) {
    dnml_status echeck; bigInt *alloc_list[3]; uint8_t alloc_cnt = 0;
    uint64_t guess_bits = (__BIGINT_COUNTDB__(a, 2) + 1) >> 1;
    BIHEAP_TEMP(guess, a->n, echeck, err, alloc_list, alloc_cnt,);
    BIHEAP_TEMP(ratio, a->n, echeck, err, alloc_list, alloc_cnt,);
    BIHEAP_TEMP(next, a->n + 1, echeck, err, alloc_list, alloc_cnt,);
    guess.limbs[(guess_bits << 6)] = 1ULL << (guess_bits % 64);
    guess.n = (guess_bits << 6) + 1;
    for (;;) {
        // next in DIVMOD_DISPATCH acts as a temporary buffer
        __BIHEAP_DIV_DISP__(a, &guess, &ratio, &next, &echeck); HEAP_OOM(echeck, err, alloc_list, alloc_cnt,);
    ...
    } __BIGINT_INTERNAL_COPY__(res, &guess); _free_alloc_list(alloc_list, alloc_cnt); *err = BIGINT_SUCCESS;
}
void __BIHEAP_NEWTON_CBRT__(bigInt *const res, const bigInt *const a, dnml_status *err) {
    ...
    BIHEAP_TEMP(guess,  (a->n + 1) << 1, echeck, err, alloc_list, alloc_cnt,);
    BIHEAP_TEMP(ratio,   a->n + 1,       echeck, err, alloc_list, alloc_cnt,);
    BIHEAP_TEMP(next,   (a->n + 1) << 1, echeck, err, alloc_list, alloc_cnt,);
    BIHEAP_TEMP(tmp,    (a->n + 1) << 1, echeck, err, alloc_list, alloc_cnt,);
    guess.limbs[(guess_bits << 6)] = 1ULL << (guess_bits % 64); guess.n = (guess_bits << 6) + 1;
    for (;;) {
        __BIHEAP_MUL_DISP__(&guess, &guess, &next, &echeck); HEAP_OOM(echeck, err, alloc_list, alloc_cnt,);
        __BIHEAP_DIV_DISP__(a, &next, &ratio, &tmp, &echeck); HEAP_OOM(echeck, err, alloc_list, alloc_cnt,);
...
void __BIHEAP_NEWTON_2NRT__(bigInt *const res, const bigInt *const a, uint64_t root, dnml_status *err) {
    ...
    BIHEAP_TEMP(guess, a->n * (root - 1), echeck, err, alloc_list, alloc_cnt,);
    BIHEAP_TEMP(ratio, a->n, echeck, err, alloc_list, alloc_cnt,);
    guess.limbs[(guess_bits << 6)] = 1ULL << (guess_bits % 64);
    guess.n = (guess_bits << 6) + 1;
    BIHEAP_TEMP(next, a->n * (root - 1), echeck, err, alloc_list, alloc_cnt,);
    BIHEAP_TEMP(xpow, a->n * (root - 1), echeck, err, alloc_list, alloc_cnt,);
    __BIHEAP_EXP_DISPATCH__(&xpow, &guess, (root - 1), &echeck); HEAP_OOM(echeck, err, alloc_list, alloc_cnt,);
    for (;;) {
        // next in DIVMOD_DISPATCH acts as a temporary buffer;
        __BIHEAP_DIV_DISP__(a, &xpow, &ratio, &next, &echeck); HEAP_OOM(echeck, err, alloc_list, alloc_cnt,);
