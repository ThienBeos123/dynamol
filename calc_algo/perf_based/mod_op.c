...
//* --------- ALGORITHMS --------- *//
//* ------------------------------ *//
void __BIGINT_CMODMUL__(const bigInt *a, const bigInt *b, const bigInt *mod, bigInt *res, calc_ctx modmul_ctx) {
    ...
    __BIGINT_MOD_DISPATCH__(&prod, mod, res, &tmp, modmul_ctx
...
void __BIGINT_BIN_MODEXP__(const bigInt *base, const bigInt *exp, const bigInt *mod, bigInt *res, calc_ctx binexp_ctx) {
    dnml_status err_check, end_stat = 0;
    size_t binexp_mark = scratch_mark(&binexp_ctx);
    BIGINT_TEMP(buf, base->n, binexp_ctx, err_check, end_stat);
    BIGINT_TEMP(tmp_res, mod->n, binexp_ctx, err_check, end_stat);
    BIGINT_TEMP(tmp_exp, exp->n, binexp_ctx, err_check, end_stat);
    BIGINT_TEMP(tmp_base, mod->n, binexp_ctx, err_check, end_stat);
    tmp_res.limbs[0] = 1; memcpy(tmp_exp.limbs, exp->limbs, exp->n * U64_BYTES);
    __BIGINT_MOD_DISPATCH__(base, mod, &tmp_base, &buf, binexp_ctx);
    while (tmp_exp.n > 0) {
        if (tmp_exp.limbs[0] & 1) {
            __BIGINT_CMODMUL__(&tmp_res, &tmp_base, mod, &tmp_res, binexp_ctx);
        } __BIGINT_CMODMUL__(&tmp_base, &tmp_base, mod, &tmp_base, binexp_ctx);
        __BIGINT_INTERNAL_RSHIFT__(&tmp_exp, 1);
    } __BIGINT_INTERNAL_COPY__(res, &tmp_res);
    scratch_reset(&binexp_ctx, binexp_mark);

}
void __BIGINT_MBIN_MODEXP__(const bigInt *base, const bigInt *exp, const bigInt *mod, bigInt *res, calc_ctx binexp_ctx) {
    //* --- 1. SETUP ---- *//
    mont_ctx modexp_contx = {
        .n      = mod,
        .nprime = __MODINV_UI64__(mod->limbs[0]),
        .k      = mod->n
    }; size_t binexp_mark = scratch_mark(&binexp_ctx), max_tsize = max((mod->n << 1), max(base->n, exp->n));
    dnml_status err_check, end_stat = 0;
    BIGINT_TEMP(r, max_tsize, binexp_ctx, err_check, end_stat); r.n = mod->n + 1;
    BIGINT_TEMP(r_mod_n, mod->n, binexp_ctx, err_check, end_stat);
    BIGINT_TEMP(tmp, max_tsize, binexp_ctx, err_check, end_stat);
    r.limbs[mod->n] = 1; __BIGINT_MOD_DISPATCH__(&r, mod, &r_mod_n, &tmp, binexp_ctx);
    __BIGINT_MUL_DISPATCH__(&r_mod_n, &r_mod_n, &tmp, binexp_ctx);
    __BIGINT_MOD_DISPATCH__(&tmp, mod, &tmp, &r, binexp_ctx);
    modexp_contx.r2 = &tmp;
    ...
    BIGINT_TEMP(tmp_base, mod->n, binexp_ctx, err_check, end_stat);
    tmp_res.limbs[0] = 1; memcpy(tmp_exp.limbs, exp->limbs, exp->n * U64_BYTES);
    __BIGINT_MOD_DISPATCH__(base, mod, &tmp_base, &tmp, binexp_ctx);
...
void __BIGINT_FIX_MODEXP__(const bigInt *base, const bigInt *exp, const bigInt *mod, bigInt *res, calc_ctx fix_ctx) {}
void __BIGINT_SLIDE_MODEXP__(const bigInt *base, const bigInt *exp, const bigInt *mod, bigInt *res, calc_ctx slide_ctx) {}
void __BIGINT_MODMUL_DISPATCH__(const bigInt *a, const bigInt *b, const bigInt *mod, bigInt *res, calc_ctx modmul_ctx) {
    if (mod->n <= BIGINT_CLASSICAL) __BIGINT_CMODMUL__(a, b, mod, res, modmul_ctx);
    else {
        dnml_status err_check, end_stat = 0;
        ...
        BIGINT_TEMP(r, mod->n + 1, modmul_ctx, err_check, end_stat); r.n = mod->n + 1;
        BIGINT_TEMP(r_mod_n, mod->n, modmul_ctx, err_check, end_stat);
        BIGINT_TEMP(tmp, mod->n << 1, modmul_ctx, err_check, end_stat);
        r.limbs[mod->n] = 1; __BIGINT_MOD_DISPATCH__(&r, mod, &r_mod_n, &tmp, modmul_ctx);
        __BIGINT_MUL_DISPATCH__(&r_mod_n, &r_mod_n, &tmp, modmul_ctx); modmul_dispatch_ctx.r2 = &tmp;
        __BIGINT_MOD_DISPATCH__(&tmp, mod, &tmp, &r, modmul_ctx);
...
void __BIGINT_MODEXP_DISPATCH__(const bigInt *base, const bigInt *exp, const bigInt *mod, bigInt *res, calc_ctx modexp_ctx) {
    if (mod->n < BIGINT_MOD_BINARY) __BIGINT_BIN_MODEXP__(base, exp, mod, res, modexp_ctx);
    else if (mod->n < BIGINT_MONT_BINARY) __BIGINT_MBIN_MODEXP__(base, exp, mod, res, modexp_ctx);
    else if (mod->n < BIGINT_MOD_FIXED) __BIGINT_FIX_MODEXP__(base, exp, mod, res, modexp_ctx);
    else __BIGINT_SLIDE_MODEXP__(base, exp, mod, res, modexp_ctx);
}
