...
void __BIGINT_FFT__(const bigInt *a, const bigInt *b, bigInt *res, calc_ctx fft_ctx, dnml_status *err) {
    ...
    bigInt *a_windows = scratch_alloc(&fft_ctx, d * (sizeof(bigInt) / U64_BYTES), &err_check);
    if (err_check == DARENA_OVERFLOW) { if (err != NULL) *err = DARENA_OVERFLOW; return; }
    bigInt *b_windows = scratch_alloc(&fft_ctx, d * (sizeof(bigInt) / U64_BYTES), &err_check);
    if (err_check == DARENA_OVERFLOW) { if (err != NULL) *err = DARENA_OVERFLOW; return; }
