...
size_t __BIGINT_MAT_TOOM3_WS__(size_t x_size, size_t z_size, size_t y_size, size_t w_size) {
    ...
    size_t yw_k = (max(y_size, w_size) + 2) / 3; /**/ size_t max_k = max(xz_k, yw_k);
...
dnml_status __BIGINT_MATMUL_TOOM3__(
    ...
    size_t x1size = (x->n > xz_k) ? min(xz_k, x->n - xz_k) : 0; // Maximum = k
    size_t z1size = (z->n > xz_k) ? min(xz_k, z->n - xz_k) : 0; // Maximum = k
    ...
    size_t y1size = (y->n > yw_k) ? min(yw_k, y->n - yw_k) : 0; // Maximum = k
    size_t w1size = (w->n > yw_k) ? min(yw_k, y->n - yw_k) : 0; // Maximum = k
    ...
    BIGINT_FTEMP(p_outer, max_k + 1, toom_ctx, toom_mark, echeck);
    BIGINT_FTEMP(p1,      max_k + 2, toom_ctx, toom_mark, echeck);
    BIGINT_FTEMP(p_neg1,  max_k +  1, toom_ctx, toom_mark, echeck);
    BIGINT_FTEMP(p_neg2,  max_k + 2, toom_ctx, toom_mark, echeck);
    // q(x) TEMPORARIES
    //  +) qOuter   = n0 + n2                           |   +) q(-1)    = qOuter - n1
    //  +) q(0)     = n0          (NO FULL TEMPORARY)   |   +) q(-2)    = 2*(q(-1) + n2) - n0
    //  +) q(1)     = qOuter + n1                       |   +) q(inf)   = n2                    (NO FULL TEMPORARY)
    BIGINT_FTEMP(q_outer, max_k + 1, toom_ctx, toom_mark, echeck);
    BIGINT_FTEMP(q_neg2,  max_k + 2, toom_ctx, toom_mark, echeck);
    BIGINT_FTEMP(q1,      max_k + 2, toom_ctx, toom_mark, echeck);
    BIGINT_FTEMP(q_neg1,  max_k + 1, toom_ctx, toom_mark, echeck);
    ...
    */
    ...
    BIGINT_FTEMP(rinf, max_m2size + max_n2size + 1, toom_ctx, toom_mark, echeck);
    ...
    /* ------------------ RECOMPOSITION ------------------ */
    BIGINT_FTEMP(tmp_res, max(x->n + z->n, y->n + w->n), toom_ctx, toom_mark, echeck); memset(tmp_res.limbs, 0, tmp_res.cap * U64_BYTES);
    __BIGINT_ADD_SHIFT__(&tmp_res, &r0, 0); __BIGINT_ADD_SHIFT__(&tmp_res, &r1, xz_k);
    __BIGINT_ADD_SHIFT__(&tmp_res, &r_neg1, (xz_k << 1)); __BIGINT_ADD_SHIFT__(&tmp_res, &r_neg2, (xz_k * 3));
    __BIGINT_ADD_SHIFT__(&tmp_res, &rinf, (xz_k << 2)); __BIGINT_INTERNAL_COPY__(xz_res, &tmp_res);
    ...
    *   +) p(inf) = m2          (NO FULL TEMPORARY) | +) q(inf) = n2          (NO FULL TEMPORARY) */
    // p(x) TEMPORARIES RESET + CALCULATIONS        // q(x) TEMPORARIES RESET + CALCULATIONS
    p_outer.n = 0; p1.n = 0; p_neg1.n = 0;          /**/ q_outer.n = 0; q1.n = 0; q_neg1.n = 0;
    p_neg2.n = 0; p_outer.sign = 1; p1.sign = 1;    /**/ q_neg2.n = 0; q_outer.sign = 1; q1.sign = 1;
    p_neg1.sign = 1; p_neg2.sign = 1;               /**/ q_neg1.sign = 1; q_neg2.sign = 1;
    memset(p_outer.limbs, 0, p_outer.cap * U64_BYTES); memset(q_outer.limbs, 0, q_outer.cap * U64_BYTES);
    memset(p1.limbs, 0, p1.cap * U64_BYTES);           memset(q1.limbs, 0, q1.cap * U64_BYTES);
    memset(p_neg1.limbs, 0, p_neg1.cap * U64_BYTES);   memset(q_neg1.limbs, 0, q_neg1.cap * U64_BYTES);
    memset(p_neg2.limbs, 0, p_neg2.cap * U64_BYTES);   memset(q_neg2.limbs, 0, q_neg2.cap * U64_BYTES);
    __BIGINT_ADD_WC__(&p_outer, &m0, &m2);              __BIGINT_ADD_WC__(&q_outer, &n0, &n2);
    __BIGINT_ADD_WC__(&p1, &p_outer, &m1);              __BIGINT_ADD_WC__(&q1, &q_outer, &n1);
    __BIGINT_SUB_SAW__(&p_neg1, &p_outer, &m1);         __BIGINT_SUB_SAW__(&q_neg1, &q_outer, &n1);
    __BIGINT_ADD_SAW__(&p_neg2, &p_neg1, &m2);          __BIGINT_ADD_SAW__(&q_neg2, &q_neg1, &n2);
    ...
    *   +) r(inf) = p(inf) * q(inf)     ---> Cap: 2k (original) --> 2k + 1 (interpolation - r4)
    */
    memset(r_neg2.limbs, 0, r_neg2.cap * U64_BYTES); memset(r_neg1.limbs, 0, r_neg1.cap * U64_BYTES);
    memset(r0.limbs, 0, r0.cap * U64_BYTES); memset(r1.limbs, 0, r1.cap * U64_BYTES); memset(rinf.limbs, 0, rinf.cap * U64_BYTES);
    __BIGINT_INTERNAL_ZSET__(&r0); __BIGINT_INTERNAL_ZSET__(&r1); __BIGINT_INTERNAL_ZSET__(&r_neg1);
    __BIGINT_INTERNAL_ZSET__(&r_neg2); __BIGINT_INTERNAL_ZSET__(&rinf);
    __BIGINT_TOOM_3__(&m0, &n0, &r0, toom_ctx, &echeck); SCRATCH_FOVF(echeck, toom_ctx, toom_mark);
    __BIGINT_TOOM_3__(&p1, &q1, &r1, toom_ctx, &echeck); SCRATCH_FOVF(echeck, toom_ctx, toom_mark);
    __BIGINT_TOOM_3__(&p_neg1, &q_neg1, &r_neg1, toom_ctx, &echeck); SCRATCH_FOVF(echeck, toom_ctx, toom_mark);
    __BIGINT_TOOM_3__(&p_neg2, &q_neg2, &r_neg2, toom_ctx, &echeck); SCRATCH_FOVF(echeck, toom_ctx, toom_mark);
    ...
    /* r1 = 2k + 8 */ __BIGINT_SUB_SAW__(&r1, &r1, &r_neg2);
    /* ------------------ RECOMPOSITION ------------------ */
    memset(tmp_res.limbs, 0, tmp_res.cap * U64_BYTES); tmp_res.n = 0; tmp_res.sign = 1;
    __BIGINT_ADD_SHIFT__(&tmp_res, &r0, 0); __BIGINT_ADD_SHIFT__(&tmp_res, &r1, yw_k);
    __BIGINT_ADD_SHIFT__(&tmp_res, &r_neg1, (yw_k << 1)); __BIGINT_ADD_SHIFT__(&tmp_res, &r_neg2, (yw_k * 3));
    __BIGINT_ADD_SHIFT__(&tmp_res, &rinf, (yw_k << 2)); __BIGINT_INTERNAL_COPY__(yw_res, &tmp_res);
