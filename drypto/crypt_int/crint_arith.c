...
dnml_status crint_mut_mulu64(crint *x, uint64_t val) {
    ...
    limb_t fsbuf[FAKE_BUF_CAP] = {UINT64_MAX}, fpbuf[FAKE_BUF_CAP + 1] = {0};
    ...
    prod = 0; prev_src_buf = 0; fake_mul = 0; pbv_crint_clear(fake_src); 
    pbv_crint_clear(fake_prod); x = 0; val = 0; return ret_stat; // clang-format on
}
dnml_status crint_mut_divu64(crint *x, uint64_t val) {
    ...
    limb_t fsbuf[FAKE_BUF_CAP] = {UINT64_C(0x104562ABC89DEF18)}, fqbuf[FAKE_BUF_CAP] = {0};
    crint fake_src = { .limbs = fsbuf, .cap = FAKE_BUF_CAP, .n = FAKE_BUF_CAP, .sign = 1, .poisoned = false };
    ...
    prev_src_buf = (_lib_crt_eq(ret_stat, CRINT_SUCCESS)) ? x->limbs : NULL;
    crint_transfer(src, quot); free(prev_src_buf); // Would lead to a standard no-op on NULL
    /* Post-operation Aggressive Cleanup */ // clang-format off
    __libdnml_smemwipe_u64(fsbuf, FAKE_BUF_CAP, 0, FAKE_BUF_CAP - 1, false);
...
dnml_status crint_mut_modu64(crint *x, uint64_t val) {
    ...
    limb_t fsbuf[FAKE_BUF_CAP] = {UINT64_C(0xFF812BBEDF12345C)}, fqbuf[FAKE_BUF_CAP] = {0};
    limb_t fake_div = UINT64_C(0x10BCDE56271CCDEF);
    crint fake_src = { .limbs = fsbuf, .cap = FAKE_BUF_CAP, .n = FAKE_BUF_CAP, .sign = 1, .poisoned = false };
    crint fake_quot = { .limbs = fqbuf, .cap = FAKE_BUF_CAP, .n = 0, .sign = 1, .poisoned = false };
    crint* src = (_lib_crt_eq(ret_stat, CRINT_SUCCESS)) ? x : &fake_src;
    crint* quot = (_lib_crt_eq(ret_stat, CRINT_SUCCESS)) ? &tmp_quot : &fake_quot;
...
dnml_status crint_mut_muli64(crint *x, int64_t val) {
    ...
    limb_t fsbuf[FAKE_BUF_CAP] = {UINT64_C(0X93426FA7CA049FB)}, fpbuf[FAKE_BUF_CAP + 1] = {0};
    crint fake_src = { .limbs = fsbuf, .cap = FAKE_BUF_CAP, .n = FAKE_BUF_CAP, .sign = 1, .poisoned = false };
    crint fake_prod = { .limbs = fpbuf, .cap = FAKE_BUF_CAP + 1, .n = 0, .sign = 1, .poisoned = false };
    ...
    reserve_stat = 0; pbv_crint_clear(tmp_prod); snew_stat = 0; src = 0;
    prod = 0; prev_src_buf = 0; fake_mul = 0; pbv_crint_clear(fake_src);
    pbv_crint_clear(fake_prod); mag_val = 0; x = 0; val = 0; return ret_stat; // clang-format on
}
...
crint crint_mulu64(crint x, uint64_t val, dnml_status *err) {
    ...
    snew_stat = 0; fake_mul = 0; src = 0; prod = 0; chosen_freed = 0; return ret_prod; // clang-format on
}
crint crint_divu64(crint x, uint64_t val, dnml_status *err) {
    ...
    limb_t fsbuf[FAKE_BUF_CAP] = {UINT64_C(0X95148E9ED7DB5BDB)}, fqbuf[FAKE_BUF_CAP] = {0};
    crint fake_src = { .limbs = fsbuf, .cap = FAKE_BUF_CAP, .n = FAKE_BUF_CAP, .sign = 1, .poisoned = false };
    crint fake_quot = { .limbs = fqbuf, .cap = FAKE_BUF_CAP, .n = 0, .sign = 1, .poisoned = false };
    crint *src = (_lib_crt_eq(ret_stat, CRINT_SUCCESS)) ? &x : &fake_src;
    ...
    __libdnml_smemwipe_u64(fsbuf, FAKE_BUF_CAP, 0, FAKE_BUF_CAP - 1, false);
    __libdnml_smemwipe_u64(fqbuf, FAKE_BUF_CAP, 0, FAKE_BUF_CAP - 1, false);
    ret_stat = 0; pbv_crint_clear(fake_src); pbv_crint_clear(fake_quot); snew_stat = 0;
    mask = 0; tmp_rem = 0; fake_div = 0; src = 0; quot = 0; chosen_freed = 0; return ret_quot; // clang-format on
}
crint crint_modu64(crint x, uint64_t val, dnml_status *err) {
    ...
    qnew_stat = 0; rnew_stat = 0; tmp_p = 0; mask = 0; tmp_rem = 0; fake_div = 0;
    src = 0; quot = 0; chosen_freed = 0; return ret_rem; // clang-format on
}
crint crint_muli64(crint x, int64_t val, dnml_status *err) {
    ...
    __libdnml_smemwipe_u64(fsbuf, FAKE_BUF_CAP, 0, FAKE_BUF_CAP - 1, false);
    __libdnml_smemwipe_u64(fpbuf, FAKE_BUF_CAP + 1, 0, FAKE_BUF_CAP, false);
    ret_stat = 0; pbv_crint_clear(fake_src); pbv_crint_clear(fake_prod); mag_val = 0;
    snew_stat = 0; fake_mul = 0; src = 0; prod = 0; chosen_freed = 0; return ret_prod; // clang-format on
}
crint crint_divi64(crint x, int64_t val, dnml_status *err) {
    ...
    __libdnml_smemwipe_u64(fsbuf, FAKE_BUF_CAP, 0, FAKE_BUF_CAP - 1, false);
    __libdnml_smemwipe_u64(fqbuf, FAKE_BUF_CAP, 0, FAKE_BUF_CAP - 1, false);
    ret_stat = 0; pbv_crint_clear(fake_src); pbv_crint_clear(fake_quot); snew_stat = 0; mag_val = 0;
    mask = 0; tmp_rem = 0; fake_div = 0; src = 0; quot = 0; chosen_freed = 0; return ret_quot; // clang-format on
}
crint crint_modi64(crint x, int64_t val, dnml_status *err) {
    ...
    ret_stat = 0; pbv_crint_clear(fake_src); pbv_crint_clear(fake_quot); 
    qnew_stat = 0; rnew_stat = 0; tmp_p = 0; mask = 0; tmp_rem = 0; fake_div = 0;
    src = 0; quot = 0; chosen_freed = 0; mag_val = 0; return ret_rem; // clang-format on
}
